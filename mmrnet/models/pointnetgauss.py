...
class GaussianSplatting(nn.Module):
    def __init__(self, feature_dim):
        ...
        self.opacity = nn.Parameter(torch.ones(1))

    def get_covariance(self, scale):
        # 直接用缩放参数创建一个对角协方差矩阵
        cov = torch.diag_embed(scale.exp() * torch.ones(3))
        return cov

    def forward(self, x, pos):
        # 降低特征维度
        if x.shape[-1] > self.feature_dim:
            x = nn.Linear(x.shape[-1], self.feature_dim)(x)

        # 计算高斯分布的协方差矩阵
        cov = self.get_covariance(self.scale)

        # 近似马氏距离
        diff = pos.unsqueeze(1) - pos.unsqueeze(0)  # [N, N, 3]
        mahalanobis_dist = torch.sum(diff ** 2 / (self.scale.exp() ** 2), dim=-1)
        weights = torch.sigmoid(-mahalanobis_dist) * self.opacity  # 使用sigmoid作为简化函数
...
class SAModule(torch.nn.Module):
    def __init__(self, ratio, r, nn, feature_dim):
        ...
        self.gaussian_splat = GaussianSplatting(feature_dim)
    ...
    def forward(self, x, pos, batch):
        ...
        # 加入高斯渲染
        x = self.gaussian_splat(x, pos[idx])
...
class GlobalSAModule(torch.nn.Module):
    ...
    def forward(self, x, pos, batch):
        ...
        return x, pos, batch
class PointNetGauss(torch.nn.Module):
